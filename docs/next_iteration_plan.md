...
## Phase G: ボール挙動とセットプレー整合の修正（2026-07-02追加） — ✅ 完了（2026-07-02）

> 「ボールが変なところでワープする」というプレイ報告の原因を調査した結果、以下3件を特定済み。G1→G2→G3 の順に実装する。

G1〜G3すべて実装済み。テスト733件全通過・型チェッククリーン。変更はすべてクライアント+テストのみ（engine無変更・Workerデプロイ不要）。
- G1（`45a579d`）: `getMissedShootRestart(events)`をbattleUtils.tsに新設し、リプレイチェーンのCK遷移チェック後・
  ゴール判定前でmissedを検出→守備側ゴールキックへ遷移。ゴールキック再配置フローを`performGoalKickRestart`に抽出して
  CK守備クリア経路と共通化（ワイプはCSS固定時間と同期のためanimSpeed非適用、COM観戦はワイプ省略、フェイルセーフ.catchの
  管轄内・clearReplayTimersは再配置完了後で8秒安全弁も有効維持）。MISSED!表示（黄）追加、軌跡result 'missed'新設で
  着弾点（枠のcol 5/16外側）に×描画、FlyingBall到達点も枠外化。テスト4件追加
- G2（`e255f15`）: OFFSIDE演出後にphase3のBALL_ACQUIREDから新保持者（GK）を特定しパス着地点→GK座標へ返却飛行、
  BATTLE_DELAY演出後も同様（coord→awardedToPieceId）。軌跡線は残さず飛行のみ。PASSIVE_TACTICSはボール移譲なしのため対象外
- G3（`c3185a6`）: `pickHeadingChanceReceiver`（攻撃側非GKのうち相手ゴールにhexDistance最寄り、同距離のみFW優先）を
  新設し、プレイヤーCKとCOM vs COM簡易CK解決の両経路に適用。旧ボール位置→受け手への飛行も追加。テスト5件追加
- 付随修正（`c172de4`）: 検証中に既存の`jwt_verify.test.ts` nbfテストが秒境界レースで約1%フレークすることを
  100回ループで特定し、`nbf: now+61`→`now+120`に修正（G2コミット時に1回赤を踏んだ原因）
- 検証: Playwright回帰 `e2e/mobile_battle_failsafe.mjs` を iPhone13/WebKit・Pixel7/Chromium 両方で全項目PASS

### タスク

**G1. 枠外シュート（missed）の処理が存在しない【最重要】**
- `src/engine/ball.ts` のシュート解決には「goal / saved_ck / missed はフェーズ外処理（ゲームエンジン側で対応）」とコメントがあるが、クライアント（Battle.tsx）には goal と saved_ck の処理しかなく、**outcome === 'missed' はコードベース全体のどこにも処理が存在しない**（grep確認済み）
- 現状の挙動: エンジンはシューターの hasBall を true のまま残す → クライアントはゴールへ飛ぶ FlyingBall を再生（軌跡 result は誤って 'success' にマップ）→ 結果表示なし → 次フレームでボールがシューターの足元にワープして攻撃続行
- 修正: ①missed 時はサッカーのルール通り守備側のゴールキック（既存の goalkick 演出フロー: setCeremony('goalkick') → GOALKICK_WIPE_COVER_MS 待機 → createGoalKickPieces → SET_BOARD → 残りワイプ → NEXT_TURN を流用。CK守備クリア経路と共通化可）②MISSED! の結果表示追加 ③軌跡 result を専用の 'missed' に修正（renderBallTrails に×等の描画追加）④FlyingBall 到達点の枠外化（優先度低）⑤**engine 側は変更しない**（フェーズ外処理の契約通り、責務はクライアント側）
- 回帰確認: goal / blocked / saved_catch / saved_ck 各経路、特に saved_ck→CKミニゲーム遷移と CK守備クリア→goalkick

**G2. オフサイド/遅延行為でボールが相手GKへ瞬間移動する**
- `turn_processor.ts` のオフサイド判定と BATTLE_DELAY は restartPiece（守備側GK）に hasBall を直接付け替えるが、クライアントにボール移動の演出がなく、旗/オーバーレイ表示の後にボールがフィールドを横断して瞬間移動する
- 修正（クライアント側のみ）: 各イベント演出の後、BALL_ACQUIRED イベントから新保持者（GK）を特定し、旧ボール位置→GK座標への launchFlyingBall を挟んでからボール表示を確定。「審判がボールを戻す」ニュアンスなので軌跡線は残さない
- 回帰確認: オフサイド後・遅延判定後にターンが正常継続（e2e フェイルセーフ非破壊）

**G3. ヘディングチャンスのボール受け手が遠くのFWにワープする**
- CKミニゲーム2ゾーン勝利時、受け手を「攻撃側のFW（位置不問）→任意の非GK」の順で選んでおり、対象がゴールから遠いとボールがそこまでワープする
- 修正: 受け手選定を「攻撃側の非GKコマのうち、相手ゴールに最も近いコマ」に変更（距離第一条件、FW優先はタイブレーク）。G2 同様に旧ボール位置からの launchFlyingBall を挟む
- 回帰確認: COM vs COM の簡易CK解決経路にも同じ選定ロジックを適用

### 品質ゲート

- 型チェック・既存テスト全通過。G1 の missed→goalkick 遷移にテスト追加（守備側GKがボール保持・専用陣形再配置）
- `e2e/mobile_battle_failsafe.mjs` 両デバイス全PASS
- ターン進行タイミング（安全弁・フェイルセーフ・8秒タイマー）との整合。G1 は `61085c0` のフェイルセーフ .catch で復旧できる位置に実装
- COM vs COM 観戦モードで演出省略しつつ盤面整合維持

---

## Phase H: セットプレー後の動的配置（2026-07-02追加） — ✅ 完了（2026-07-02）

実装済み（`16e208b`）。テスト734件全通過・型チェッククリーン。変更はクライアント+テストのみ（Workerデプロイ不要）。
- `createSetPieceRestartPieces` を battleUtils.ts に実装し旧 createGoalKickPieces を削除。
  第1層アンカー=編成座標をcreateInitialPiecesから導出し線形リマップ（守備側: goalkick[4,14]/fk・pk_fail[6,15]、
  攻撃側[13,21]、GK自ゴール前固定、交代出場コマはDEFAULT_TEMPLATE同ポジで代替）。
  第2層状況シフト=ビハインド+1（残り25%以下+2）/リード終盤-1、FP深度[2,21]クランプ。
  第3層揺らぎ=mulberry32（シード=ターン番号+コマindex、Math.random不使用で決定的）で列・行±1
- 適用4+1箇所: G1枠外シュート/CK守備クリア（プレイヤー+COM観戦）=goalkick、FK失敗=fk_fail、PK失敗=pk_fail
  （FK/PK失敗は再配置なし→performGoalKickRestart経由でワイプ演出も統一適用、.catchフェイルセーフ付き）、
  COM観戦のFK/PK失敗も同アルゴリズム（ワイプ省略）。ゴール後キックオフは対象外（現行維持）
- テスト: 品質ゲート(a)〜(e)+restartType差の6件構成（733→734）
- 検証: `e2e/mobile_battle_failsafe.mjs` 両デバイス全PASS。COM観戦フルマッチスモーク（Playwright）で
  コンソールエラー0件のままFULL TIME到達を確認

### 背景と設計判断（議論済み・確定）

- 現状の createGoalKickPieces は固定の深度テーブル（DEF_DEPTH/ATK_DEPTH）+固定の列テーブル（COLS）で全コマを配置するため、(1) 毎回100%同じ配置で単調、(2) プレイヤーが編成で組んだフォーメーション（4-4-2 / 3-5-2 等）を完全に無視する、という2つの問題がある
- ユーザー決定: セットプレーごとに毎回同じ定型配置になるのは単調なので、状況に応じて毎回変化する配置アルゴリズムを導入する
- ミニゲーム前の配置演出は不要（ミニゲームは確率判定の代替UIであり盤面はほぼ見えないため）。対象は「ミニゲーム/判定の後、盤面に戻る瞬間の配置」のみ

### 実装仕様: 3層アルゴリズム（アンカー + 状況シフト + 揺らぎ）

新関数 `createSetPieceRestartPieces` を battleUtils.ts に実装し、既存の createGoalKickPieces を置き換える。

**入力**: currentPieces, 守備側チーム（ボールを得る側）, restartType（'goalkick' | 'fk_fail' | 'pk_fail'）, formationData, opponent情報, seed（ターン番号）, スコア・現在ターン・全ターン数

**第1層: アンカー（フォーメーション由来の基準位置）**
- 基準位置を固定テーブルではなくそのチームの編成フォーメーションから導出（自チーム=formationData.starters、相手=createInitialPiecesが使うテンプレート/プリセット座標）
- 守備側: 編成の形を保ったまま自陣側に圧縮（縦方向を「GKライン〜ハーフライン手前」レンジへ線形リマップ）
- 攻撃側: ハーフライン付近まで前進圧縮（GKは自陣ゴール前のまま）
- restartType差: 'goalkick'=守備側深め / 'fk_fail'・'pk_fail'=GKキャッチの流れなので守備側の圧縮を1〜2列浅く

**第2層: 状況シフト（スコアと残り時間）**
- ビハインド: 全体+1列（残り25%以下なら+2列）/ リード終盤のみ-1列 / 同点はシフトなし
- GKは自ゴール前固定、FPは自陣外周〜敵陣にはみ出さない範囲へクランプ

**第3層: 揺らぎ（シード付き乱数）**
- 各FPに列・行 -1/0/+1 の小オフセット。Math.random()ではなくmulberry32等のシード付きPRNGで決定的に生成（将来オンラインでサーバー/両クライアントが同一配置を再現するため）
- 既存の衝突回避（occupied Set）流用、盤面範囲（col 0-21, row 0-33）クランプ厳守
- ボールは守備側GK（不在時は守備側の任意コマ）

### 適用箇所（createGoalKickPieces の全呼び出しを置換）

1. G1 枠外シュート → performGoalKickRestart 経由（'goalkick'）
2. CK守備クリア（プレイヤー経路 + COM vs COM 簡易解決経路）（'goalkick'）
3. FK失敗（handleFKComplete）: 現状はsetBallHolderのみ → 新アルゴリズムで再配置+ワイプ演出を適用（'fk_fail'）
4. PK失敗（handlePKComplete）: 同上（'pk_fail'）
- 旧 createGoalKickPieces は削除。ゴール後キックオフ（createGoalRestartPieces）は対象外

### 品質ゲート

- 型チェック・既存テスト全通過。新規テスト: (a)同一seed同一状況で決定的 (b)異なるseedで配置変化 (c)盤面範囲内・重なりなし・GK自ゴール前・ボール守備側GK (d)フォーメーション相対形状の保存（左WGは左サイド） (e)ビハインド終盤は同点時より前方
- `e2e/mobile_battle_failsafe.mjs` 両デバイス全PASS
- 61085c0フェイルセーフ・8秒安全弁・ワイプタイミング（animSpeed非適用の件含む）の整合維持
