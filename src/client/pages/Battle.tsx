...
export default function Battle({ onNavigate, matchId, gameMode, authToken, myTeam: propMyTeam, formationData, onMatchEnd, comDifficulty = 'regular' }: BattleProps) {
  ...
  const [goalCelebration, setGoalCelebration] = useState<GoalCelebrationInfo | null>(null);
  ...
  const highlightHexes = useMemo(() => {
    ...
    // ボール操作モード中は移動範囲（緑）を出さない（選択中の行動ハイライトに集中）
    if (state.actionMode === 'pass' || state.actionMode === 'shoot' || state.actionMode === 'throughPass') return [];
  ...
  }, [selectedPiece, isInputDisabled, boardContext, state.actionMode]);

  // ── パスモード: パス可能な味方コマ（青リング） ──
  const passTargetHexes = useMemo(() => {
    if (!selectedPiece || isInputDisabled || state.actionMode !== 'pass') return [];
    const accurateRange = getAccuratePassRange(selectedPiece);
    return state.board.pieces
      .filter(
        (p) =>
          ...
          !p.isBench &&
          hexDistance(selectedPiece.coord, p.coord) <= accurateRange,
      )
      .map((p) => ({ col: p.coord.col, row: p.coord.row }));
  }, [selectedPiece, isInputDisabled, state.actionMode, state.board.pieces, state.myTeam]);

  // ── スルーパスモード: スルーパス可能な空きHEX（シアン） ──
  const throughPassHexes = useMemo(() => {
    if (!selectedPiece || isInputDisabled || state.actionMode !== 'throughPass') return [];
    const accurateRange = getAccuratePassRange(selectedPiece);
    const occupied = new Set(
      state.board.pieces.filter((p) => !p.isBench).map((p) => `${p.coord.col},${p.coord.row}`),
    );
    const pos = selectedPiece.position;
    const result: HexCoord[] = [];
    const { col: sc, row: sr } = selectedPiece.coord;
    for (let col = sc - accurateRange; col <= sc + accurateRange; col++) {
      for (let row = sr - accurateRange; row <= sr + accurateRange; row++) {
        const coord = { col, row };
        if (!boardContext.isValidHex(coord)) continue;
        if (occupied.has(`${col},${row}`)) continue;
        if (hexDistance(selectedPiece.coord, coord) > accurateRange) continue;
        if (isShootZoneForPiece(coord, state.myTeam, pos)) continue;
        result.push(coord);
      }
    }
    return result;
  }, [selectedPiece, isInputDisabled, state.actionMode, state.board.pieces, state.myTeam, boardContext]);
  ...
  const shootRangeHexes = useMemo(() => {
    ...
    if (state.actionMode !== 'shoot') return [];
  ...
  useEffect(() => {
    ...
    const handleKey = (e: KeyboardEvent) => {
      ...
      switch (e.key.toLowerCase()) {
        ...
        case 't': // スルーパスモード
          if (selectedPiece?.hasBall) {
            dispatch({ type: 'SET_ACTION_MODE', mode: state.actionMode === 'throughPass' ? null : 'throughPass' });
          }
          break;
        case 'w': // シュートモード
          if (selectedPiece?.hasBall) {
  ...
  const handleHexClick = useCallback(
    (coord: HexCoord) => {
      ...
      const isInHexes = (hexes: HexCoord[]) => hexes.some((h) => h.col === coord.col && h.row === coord.row);

      // ── パスモード: 青リングの味方のみ反応 ──
      if (state.actionMode === 'pass') {
        if (!isInHexes(passTargetHexes)) return;
        ...
        if (!hasBall || !isInHexes(throughPassHexes)) return;
        dispatch({ type: 'THROUGH_PASS', fromPieceId: state.selectedPieceId, targetHex: coord });
      } else if (hasBall) {
        // ボール保持者 + モード未選択では誤操作防止のため予約しない。
        return;
      } else {
        // ボール非保持者 → 移動
        if (!isInHexes(highlightHexes)) return;
        dispatch({ type: 'ADD_ORDER', order: { pieceId: state.selectedPieceId, action: 'move', targetHex: coord } });
      }
        ...
        (async () => {
          ...
          if (ckShoot) {
            ...
            if (isComVsCom) {
              ...
              dispatch({
                type: 'SET_BOARD',
                board: { pieces: boardPieces, freeBallHex },
                turn: state.turn,
                scoreHome: newScoreHome,
                scoreAway: newScoreAway,
              });
          ...
          if (goalScoredRef.current.scored) {
            setGoalCelebration({
              team: goalScoredRef.current.scorerTeam ?? 'home',
              scoreHome: newScoreHome,
              scoreAway: newScoreAway,
            });
  ...
  const handleCKComplete = useCallback(async (data: import('../components/minigame/CKGame').CKInput) => {
    ...
    if (attackWins === 3) {
      ...
      setGoalCelebration({ team: attackTeam, scoreHome: newScoreHome, scoreAway: newScoreAway });
      soundManager.playGoalCelebration();
      setCeremony('goal');
      await wait(GOAL_CEREMONY_MS);
      setCeremony(null);
      ...
      dispatch({
        type: 'SET_BOARD',
        board: { pieces: resetPieces },
        turn: state.turn, scoreHome: newScoreHome, scoreAway: newScoreAway,
      });
      dispatch({ type: 'NEXT_TURN' });
      return;
    }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        <div style={{ flex: 1, position: 'relative', minHeight: 0 }} ref={boardRef}>
          ...
          <div style={{
            ...
            {unorderedPieces.map((piece) => {
              const isSelectedQuick = piece.id === state.selectedPieceId;
              return (
                <button
                  key={piece.id}
                  onClick={(e) => { e.stopPropagation(); handleSelectPiece(piece.id); }}
                  style={{
                    width: isSelectedQuick ? 38 : 30,
                    height: isSelectedQuick ? 38 : 30,
                    borderRadius: '50%',
                    border: isSelectedQuick
                      ? '2px solid #facc15'
                      : `2px solid ${POSITION_COLORS[piece.position]}60`,
                    background: isSelectedQuick ? 'rgba(250,204,21,0.18)' : 'rgba(20,20,40,0.85)',
                    color: isSelectedQuick ? '#fff' : POSITION_COLORS[piece.position],
                    fontSize: isSelectedQuick ? 10 : 9,
                    fontWeight: 'bold',
                    cursor: 'pointer',
                    padding: 0,
                    flexShrink: 0,
                    boxShadow: isSelectedQuick ? '0 0 12px rgba(250,204,21,0.55)' : undefined,
                    transition: 'width 0.12s ease-out, height 0.12s ease-out, background 0.12s',
                  }}
                >
                  {piece.position}
                </button>
        ...
        <ActionBar
          ...
          onRemoveSelectedOrder={() => {
            if (selectedPiece) dispatch({ type: 'REMOVE_ORDER', pieceId: selectedPiece.id });
          }}
